						...
						if (id_RP == getIndex)
						{
							...
							for (int i = 22; i <= 85; i++)
							{
								if (int.Parse(sELeditCache.Instance.sELeditDatas.eLC.GetValue(69, k, i)) > 0)
								{
									...
									recipeItem = new recipeItem();
									recipeItem.ID = int.Parse(sELeditCache.Instance.sELeditDatas.eLC.GetValue(69, k, i));
									recipeItem.QTD = int.Parse(sELeditCache.Instance.sELeditDatas.eLC.GetValue(69, k, i + 1));
									flowLayoutPanel_itemrecipe.Controls.Add(recipeItem);
									i++;
								}
