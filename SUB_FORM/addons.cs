		...
		private Image Recip(int Id)
		{
			...
			try
			{
				...
				for (int ef = 0; ef < sELeditCache.Instance.sELeditDatas.eLC.Lists[La].elementValues.Length; ef++)
				{
					...
					if (Id == a)
					{
						string image = sELeditCache.Instance.sELeditDatas.eLC.GetValue(La, ef, pos_item);

						imgg = Img(int.Parse(sELeditCache.Instance.sELeditDatas.eLC.GetValue(La, ef, pos_item)));
						break;
					}
				}
				return imgg;
